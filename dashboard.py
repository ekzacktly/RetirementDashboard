...
# ==========================================
# SESSION STATE INITIALIZATION (RESET KEY ARCHITECTURE)
# ==========================================
...
penalty_age = st.sidebar.number_input("Early Withdrawal Penalty Age", value=int(config.get("penalty_age", 60)), step=1,
...
use_smile_model = st.sidebar.checkbox("Use Retirement Spending Smile", value=bool(config.get("use_smile_model", True)),
...
# ==========================================
# HELPER FUNCTIONS (WITH INFLATION MULTIPLIERS)
# ==========================================
...
def calc_fed_tax(gross: float, inf_mult: float = 1.0, is_penalized: bool = False, pen_pct: float = 0.10,
...
def calc_taxable_ss(ss_amount: float, magi: float, is_single_filer: bool, inf_mult: float = 1.0) -> float:
    t1 = (25000.0 if is_single_filer else 32000.0) * inf_mult
    t2 = (34000.0 if is_single_filer else 44000.0) * inf_mult
    prov_inc = magi + 0.5 * ss_amount
    if prov_inc <= t1:
        return 0.0

    taxable = 0.0
    if prov_inc > t1:
        taxable += 0.50 * min(prov_inc - t1, t2 - t1)
    if prov_inc > t2:
        taxable += 0.85 * (prov_inc - t2)

    return float(min(taxable, 0.85 * ss_amount))
...
def get_gross_for_net_total(current_spend: float, ss_amount: float, state_tax_dec: float, state_exempts: bool,
    ...
    def net_cash_total(g: float) -> float:
        taxable_ss = calc_taxable_ss(ss_amount, g, is_single_filer, inf_mult)
        fed_tax = calc_fed_tax(g + taxable_ss, inf_mult, is_penalized, pen_pct, is_single_filer)
        state_tax = 0.0 if state_exempts else (g * state_tax_dec)
        return (g + ss_amount) - fed_tax - state_tax
...
def withdraw_from_dual_brokerage_dynamic(net_needed: float, b1: float, basis1: float, b2: float, basis2: float,
    ...
    def net_yield(gross_draw: float) -> float:
        ...
        new_taxable_ss = calc_taxable_ss(ss_amount, ordinary_gross + gains, is_single_filer, inf_mult)
        new_ord_tax = calc_fed_tax(ordinary_gross + new_taxable_ss, inf_mult, False, 0.0, is_single_filer)
        base_ord_tax = calc_fed_tax(
            ordinary_gross + calc_taxable_ss(ss_amount, ordinary_gross, is_single_filer, inf_mult), inf_mult, False,
            0.0, is_single_filer)
    ...
    if net_needed <= max_net:
        ...
        new_taxable_ss = calc_taxable_ss(ss_amount, ordinary_gross + gains_realized, is_single_filer, inf_mult)
        new_ord_tax = calc_fed_tax(ordinary_gross + new_taxable_ss, inf_mult, False, 0.0, is_single_filer)
        base_ord_tax = calc_fed_tax(
            ordinary_gross + calc_taxable_ss(ss_amount, ordinary_gross, is_single_filer, inf_mult), inf_mult, False,
            0.0, is_single_filer)
...
with tab1:
    ...
    if use_ss:
        ss_payout_scenario = st.selectbox(
            ...
            index=["100% (Scheduled Benefits)", "78% (2032 SSA Depletion Cut)", "74% (2032 CBO Depletion Cut)"].index(
    ...
    if asset_strategy == "Unified Portfolio":
        ...
        with u_col1:
            pre_ret_return = st.number_input("Pre-Retirement Return (Real %)", value=float(config["pre_ret_return"]),
                                             step=0.1,
                                             help="Expected real investment return before retirement.") / 100.0
        with u_col2:
            post_ret_return = st.number_input("Post-Retirement Return (Real %)", value=float(config["post_ret_return"]),
                                              step=0.1,
        ...
        with al_col2:
            ...
            pre_ret_roth = st.number_input("Pre-Ret Return (%)  ", value=float(config.get("pre_ret_roth", 8.5)),
                                           step=0.1, help="Expected real return for tax-free Roth accounts.") / 100.0
    ...
    with col3a:
        ...
        p1_trad_ira_mo = st.number_input("P1 Trad IRA Monthly ($)", value=float(config["p1_trad_ira_mo"]), step=100.0,
    ...
    with col3b:
        ...
        p2_trad_ira_mo = st.number_input("P2 Trad IRA Monthly ($)", value=float(config["p2_trad_ira_mo"]), step=100.0,
                                         disabled=is_single,
    ...
    with col4a:
        ...
        p1_roth_401k_start = st.number_input("P1 Roth 401(k) Start Bal", value=float(config["p1_roth_401k_start"]),
                                             step=5000.0, help="Current balance in Person 1's designated Roth 401(k).")
        ...
        p1_roth_ira_mo = st.number_input("P1 Roth IRA Monthly ($)", value=float(config["p1_roth_ira_mo"]), step=100.0,
    ...
    with col4b:
        ...
        p2_roth_ira_mo = st.number_input("P2 Roth IRA Monthly ($)", value=float(config["p2_roth_ira_mo"]), step=100.0,
    ...
    # ==========================================
    # JSON PROFILE EXPORTER (PLACED IN SIDEBAR CONTAINER)
    # ==========================================
    current_settings = {
    ...
    def run_simulation(test_gross_income: float, return_data: bool = False, allow_negative_brokerage: bool = False,
        ...
        inf_mult_yr0 = 1.0
        base_net_spend = test_gross_income - calc_fed_tax(test_gross_income, inf_mult_yr0, False, 0.0, is_single)
        ...
        for age in range(current_age, max_sim_age):
            ...
            if not is_retired:
                ...
                taxable_ss_base = calc_taxable_ss(ss_yr, 0.0, is_single, inf_mult)
                base_tax_fed = calc_fed_tax(taxable_ss_base, inf_mult, False, 0.0, is_single)
                ...
                if net_ss >= current_spend:
                    ...
                    roth_in_yr += surplus
                    p1_roth_bal += surplus / 2.0
                    p2_roth_bal += surplus / 2.0
                    ...
                    if shortfall > 0:
                        net_brok_drawn, brok_tax_fed, brok_tax_state, marginal_ord_tax, gains_realized, p1_brok_bal, p1_brok_basis, p2_brok_bal, p2_brok_basis = withdraw_from_dual_brokerage_dynamic(
                            shortfall, p1_brok_bal, p1_brok_basis, p2_brok_bal, p2_brok_basis, ordinary_gross, ss_yr,
                            state_tax_decimal, inf_mult, is_single, allow_negative_brokerage)
                    ...
                    if shortfall > 0 and trad_bal_tot > 0:
                        ...
                        new_taxable_ss = calc_taxable_ss(ss_yr, ordinary_gross + take_trad + gains_realized, is_single,
                                                         inf_mult)
                        new_ord_tax = calc_fed_tax(ordinary_gross + take_trad + new_taxable_ss, inf_mult, True,
                                                   penalty_pct, is_single)
                        base_ord_tax_existing = calc_fed_tax(
                            ordinary_gross + calc_taxable_ss(ss_yr, ordinary_gross + gains_realized, is_single,
                                                             inf_mult), inf_mult, False, 0.0, is_single)
                ...
                taxable_ss = calc_taxable_ss(ss_yr, gross_trad, is_single, inf_mult)
                trad_tax_fed = calc_fed_tax(gross_trad + taxable_ss, inf_mult, False, 0.0, is_single)
                ...
                if net_trad_and_ss >= current_spend:
                    ...
                    if age < rmd_start_age:
                        ...
                        p2_roth_bal += surplus / 2.0
                        roth_in_yr = surplus
                        total_roth_conv_net += surplus
                        total_tax_roth += trad_tax_fed * ratio_surplus
                    else:
...
with tab2:
    ...
    *   **Taxable (Non-Qualified Brokerage):** 
    ...
    ## 2. Advanced Institutional Constraints

    *   **Social Security & The "Tax Torpedo":**
        *   **Claiming Mechanics:** The engine mathematically evaluates claiming ages against a Full Retirement Age (FRA) of 67. Claiming at age 62 reduces your check by exactly 30%. Conversely, if you delay until age 70, the engine awards an 8% increase per year, boosting your FRA baseline by exactly 24%. 
        *   **Primary Insurance Amount (PIA) Approximation:** The engine provides a live "Rule of Thumb" estimate of your FRA baseline benefit inside the tooltips. This is generated by approximating your Average Indexed Monthly Earnings (AIME) from your current salary and passing it through the progressive **2026 IRS Bend Points**:
            *   90% of your first \$1,286 of AIME
            *   32% of your AIME between \$1,286 and \$7,749
            *   15% of your AIME above \$7,749
        *   **Provisional Income Rules:** Social Security is not taxed like ordinary income. The engine evaluates your "Provisional Income" (MAGI + 50% of your SS benefits) every single year. For a married couple filing jointly, if this number crosses \$32,000, up to 50% of your benefits become taxable. If it crosses \$44,000, up to 85% of your benefits become taxable.
        *   **The Torpedo Effect:** The engine dynamically recalculates Provisional Income on the fly. If you pull money from your Brokerage account, the capital gains will spike your MAGI, which in turn forces more of your Social Security to become taxable. The engine correctly models this compounding "Torpedo" penalty to evaluate exactly how much gross withdrawal is required.
    *   **Asset Location Strategies:** Sophisticated wealth managers do not hold identical asset allocations across different account types. This engine allows you to decouple returns and assign mathematically distinct Glide Profiles to each tax bucket:
        *   **Safe (Standard TDF):** Starts shifting 25 years before retirement and stabilizes 7 years after. Best for Pre-Tax accounts to mitigate immediate Sequence of Returns Risk.
        *   **Moderate (Taxable Bridge):** Starts shifting 15 years before retirement. Best for Brokerage accounts providing flexible bridge income.
        *   **Delayed (Roth Heritage):** Doesn't start shifting until the exact year of retirement, gliding slowly over 20 years. Best for Roth accounts left to compound tax-free.
        *   **No Glide (Abrupt Drop):** The portfolio return drops instantly at retirement age with no smoothing curve.
    *   **Chained CPI Bracket Indexing:** Over a 40-year retirement, static tax brackets cause artificial "bracket creep," inflating your taxes simply due to currency devaluation. If toggled, the engine uses your selected inflation rate to dynamically expand the IRS Standard Deductions, Ordinary Brackets, and Capital Gains Brackets every year.
    *   **Medicare IRMAA Cliffs:** If your Modified Adjusted Gross Income (MAGI) crosses specific thresholds after age 65, Medicare assesses a punitive Income-Related Monthly Adjustment Amount (IRMAA). The engine models this exact cliff penalty and forces an end-of-year withdrawal from your Roth or Brokerage accounts to pay the IRS surcharge.
    ...
    ## 4. Decumulation Waterfall: The Three-Phase Lifecycle
    ...
    ## 5. Mathematical Sub-Engines & Algorithmic Rules

    ### A. Dynamic Spending Multiplier (The Spending Smile)
    ...
    *   **Ages 86–95 (Care Curl):** Increases by $1.0\%$ per year to accommodate assisted living, ending at $90\%$ by age 95.
    ...
    ### B. Progressive Tax Engine & Inverse Net-to-Gross Solver
    ...
    *   **Inverse Net-to-Gross Solver:** To satisfy an after-tax living target $N$, the engine executes a bounded Brent's root-finding solver (`scipy.optimize.root_scalar`) across the function:
    ...
    ### C. Statutory Required Minimum Distributions (RMDs)
...
with tab3:
    ...
    if current_age < retire_age:
        st.markdown(
            ...
            f"and your total portfolio reached **\${portfolio_at_retire_val:,.0f}** upon retirement at age {retire_age}.")
    else:
    ...
    if retire_age < penalty_age:
        ...
        if tot_penalties_val > 0:
            st.markdown(
                f"Because you retired early at age {retire_age}, the engine attempted to fund your lifestyle using your Post-Tax (Roth) and Taxable Brokerage accounts to avoid the 10% IRS penalty on Pre-Tax withdrawals. However, those funds were not enough to bridge the gap, and you were forced to incur **\${tot_penalties_val:,.0f}** in early withdrawal penalties.")
        else:
            st.markdown(
    ...
    if str(depletion_age_val) != "Never" and int(depletion_age_val) <= target_lifespan:
        st.error(
            f"Unfortunately, your portfolio depleted entirely at age **{depletion_age_val}**. You ran out of money before reaching your target lifespan of {target_lifespan}.")
    else:
        st.success(
